...
#### Interrupts 中止机制
*注意不是终止.*  
有时候程序必须要暂停去处理一些需要相应的重要事件. 计算机硬件提供了一种叫做**中止(interrupts)**的机制去处理(handle)这些事件. 比如鼠标移动了, 计算机需要暂停程序运行, 去移动屏幕上的光标. 这时终止请求会被的递交给*interrupt handler(中止控制器)*, 中止控制器会处理中止. 每种不同类型的中止用不同的整数来表示. 在物理内存的开始处, 有一个*中止向量表(table of interrupt vectors)*, 其中包含了中止控制器的分块地址. 刚才说的用来表示中止类型的整数是表中的必不可少的索引值.  

*下面这段中出现的"发起", 都翻译子raised from这个词, 感觉有点像raise exceptions的那个意思.*  
CPU外还可以发起*外部中止(external interrupts)*, 其实鼠标属于这种外部发起的. 很多的I/O设备都会发起外部中止.  
相对的, 内部中止在CPU内部被发起, 无论是CPU内部错误还是直接发起的中止指令, 其中错误中止被叫做*traps*. 产生中止的中止指令被叫做*软件中止(software interrupts)*. 比如DOS提供了各种各样的中止用的API, 现代的操作系统可以直接调用C提供的接口.  

很多中止控制器会在引起本次中止的程序运行完毕后返回. 它们会恢复中止前的所有寄存器值. 所以, 中止控制器发生前后看起来就好像跟什么都没发生过一样, 除了损失了一些时钟周期. *traps*在通常情况下不会返回, 他们通常会中止程序运行. 