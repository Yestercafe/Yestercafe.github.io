...
接下来从0x000A0000到0x000FFFFF的384KB是提供给硬件使用的, 比如视频显示缓冲区(video display buffers, 这个buffers我是非常不想翻译的...),  还有保存在NVM中的固件(原文是and firmware held in non-volatile memory). NVM这东西我也是第一次简单, 并不是很了解, 所以有请[百度百科][3]:  
